...
namespace ImDui
{
	...
	struct D2DRender
	{
		...
		bool Init(ID2D1Factory* pD2DFactory, IDWriteFactory* pDWriteFactory, IWICImagingFactory* pWICFactory, ID2D1DeviceContext1* pMainRT)
		{
	...
	void InitResources(ID2D1Factory* pD2DFactory, IDWriteFactory* pDWriteFactory, IWICImagingFactory* pWICFactory, ID2D1DeviceContext1* pMainRT, HWND target)
	{
	...
	void ProcessInput()
	{
		if (GetForegroundWindow() != target_wnd)
			return;
		...
		events.MousePos.x = cursor_pos.x;
		events.MousePos.y = cursor_pos.y;
	}
